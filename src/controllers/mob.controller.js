...
const createMob = async (req, res) => {
    try {
        const { name, imageUrl, health, translations } = req.body;
        
        const newMob = await mobService.createMob({ 
            name, 
            imageUrl, 
            health, 
            translations 
...
const updateMob = async (req, res) => {
    try {
        ...
        const updatedMob = await mobService.updateMob(Number(id), { 
            name, 
            imageUrl, 
            health, 
            translations 
        ...
        res.status(500).json({ error: "Error al actualizar el Mob" });
    }
};
